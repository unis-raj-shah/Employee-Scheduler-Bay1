...
def get_orders_for_scheduling(target_date: Optional[datetime] = None):
    ...
    try:
        ...
        # Debug output
        print(f"Cases to pick calculation for {date_str}:")
    ...
    for operation, roles in nested_roles.items():
        if isinstance(roles, dict):
            for role, count in roles.items():
                # Replace spaces with underscores in role names for consistency
                clean_role = role.replace(' ', '_')
                # Use operation prefix to make roles unique
                role_key = f"{operation}_{clean_role}" if operation != "replenishment" else clean_role
